...
void i2c_byte_write_frame (unsigned char sa,unsigned char mr,unsigned char data)
{
	...
	if (I2STAT!=0X08)
	{
		uart0_tx_string("write_Error in start condition!\n");
		goto exit;
	}
	I2DAT=sa; //Send SA+W
	I2CONCLR=(1<<3); //SI=0
	...
	if (I2STAT!=0X18)
	{
		uart0_tx_string("write_Error in SA!\n");
		goto exit;
	}
	I2DAT=mr; // Send MA
	I2CONCLR=(1<<3); //SI=0
	...
	if (I2STAT!=0X28)
	{
		uart0_tx_string("write_Error in MA!\n");
		goto exit;
	}
	...
	if (I2STAT!=0X28)
	{
		uart0_tx_string("write_Error in DATA!\n");
		goto exit;
	}
	exit:
		I2CONSET=(1<<4); //STO=1
...
unsigned char i2c_byte_read_frame(unsigned char sa, unsigned char mr)
{
	...
	if(I2STAT!=0x40)
	{
		uart0_tx_string("read_Error in SA+R!\r\n");
		goto exit;
	}
	I2CONCLR=(1<<3); //SI=0
	while (SI==0);
